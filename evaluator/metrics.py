...
#HR@K 的意义是：在所有用户中，推荐列表中至少包含一个真实交互项的用户比例。
...
#MRR 衡量的是第一个命中项的 倒数排名（reciprocal rank）的平均值，越靠前的命中项会对 MRR 有更高的贡献。
...
class NDCG(TopkMetric):
    ...
    def calculate_metric(self, dataobject):
        ...
        metric_dict['c-fair']=metric_dict['ndcg_Std@20']/metric_dict['ndcg@20']
        return metric_dict
...
class P_Fair(AbstractMetric):
    r""" """

    metric_type = EvaluatorType.RANKING
    metric_need = ["rec.items", "data.item_provider"]
    ...
    def used_info(self, dataobject):
        """Get the matrix of recommendation items and number of items in total item set"""
        item_matrix = dataobject.get("rec.items")
        item_provider = dataobject.get("data.item_provider")
        return item_matrix.numpy(), item_provider.numpy()

    def calculate_metric(self, dataobject):
        item_matrix, item_provider = self.used_info(dataobject)
        metric_dict = {}
        for k in self.topk:
            key = "{}@{}".format("P-Fair-CV", k)
            fairness_score = self.get_fairness(item_matrix[:, :k], item_provider)
            metric_dict[key] = round(fairness_score, self.decimal_place)
        return metric_dict

    def get_fairness(self, item_matrix, item_provider):
        """ """
        ...
        for _, rec_u in enumerate(item_matrix):
            tmp_set = set()
            for i in range(k):
                exposure_score[rec_u[i]] += score[i]
                tmp_set.add(item_provider[rec_u[i]])
...
class EO(TopkMetric):
    ...
    def calculate_metric(self, dataobject):
        pos_index, pos_len = self.used_info(dataobject)
        sensitive=dataobject.get("data.age")
        sensitive = sensitive[:6400]
        result = self.metric_info(pos_index, pos_len)
        #result=result[-6400:]
        #在这里将result进行划分
        unique_sensitives = torch.unique(sensitive)
        fairness_results = {}
        metric_dict={}
        for value in unique_sensitives:
    # 获取当前敏感属性值对应的用户索引
           indices = (sensitive == value)
    # 根据这些索引选择对应的 `result`
           group_result = result[indices]
    # 计算该组的 NDCG 值（这里假设 `self.topk_result` 用于计算 NDCG）
           group_metric_dict = self.topk_result("ndcg", group_result)
    # 将该组的结果存入字典
           fairness_results[f"sensitive_{value.item()}"] = group_metric_dict
           metric_dict = self.topk_result("ndcg", result)
        # 提取每个组的 ndcg@20 值
        ndcg_values = [group['ndcg@20'] for group in fairness_results.values()]
# 计算这些值的标准差
        ndcg_std = np.std(ndcg_values)
        metric_dict['eo']=ndcg_std
        return metric_dict
