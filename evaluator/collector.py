from evaluator.register import Register
import torch
import copy
...
class Collector(object):
    ...
    def data_collect(self, train_data):
        ...
        if self.register.need("data.item_provider"):
            self.data_struct.set(
                "data.item_provider",
                train_data.dataset.get_item_feature()[self.config["PRODIVER_ID_FIELD"]],
            )
    ...
    def eval_batch_collect(
        ...
        # if True:
        #    user_age = interaction["age"]  # 获取用户的年龄信息
        #    self.data_struct.update_tensor("data.age", user_age.to(self.device))
    ...
    def get_data_struct(self):
        ...
        #"rec.score"
        for key in ["rec.topk", "rec.meanrank", "rec.items", "data.label"]:
